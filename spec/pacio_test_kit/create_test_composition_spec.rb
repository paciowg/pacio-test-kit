RSpec.describe PacioTestKit::CreateTest do
  ...
  let(:document_reference_json_returned) do
    JSON.parse(
      File.read(File.join(__dir__, '..', 'fixtures', 'adi_document_reference_response.json'))
    )
  end
  ...
  it 'fails if the response status is not 201' do
    stub_request(:post, "#{url}/#{resource_type}")
      .to_return(status: 500, body: document_reference_json_returned.to_json, headers: response_headers)
    result = run(runnable, resource_input: document_reference.to_json, url:)
    expect(result.result).to eq('fail')
  ...
  it 'fails if the response resource id is missing' do
    body = { resourceType: 'DocumentReference', meta: { lastUpdated: 'now' } }.to_json
  ...
  it 'fails if the response resource id is the same as the submitted resource id' do
    body = { resourceType: 'DocumentReference', id: resource_id, meta: { lastUpdated: 'now' } }.to_json
    stub_request(:post, "#{url}/#{resource_type}").to_return(status: 201, body:, headers: response_headers)
  ...
  it 'fails if response resource meta.lastUpdated is present and is the same as submitted resource meta.lastUpdated' do
    body = { resourceType: 'DocumentReference', id: '567', meta: { lastUpdated: 'now' } }.to_json
    stub_request(:post, "#{url}/#{resource_type}").to_return(status: 201, body:, headers: response_headers)

    resource_input = { resourceType: 'DocumentReference', id: '123', meta: { lastUpdated: 'now' } }.to_json
    result = run(runnable, resource_input:, url:)
    expect(result.result).to eq('fail')
  ...
  it 'fails if location header is not returned' do
    body = { resourceType: 'DocumentReference', id: '456', meta: { lastUpdated: 'now' } }.to_json
