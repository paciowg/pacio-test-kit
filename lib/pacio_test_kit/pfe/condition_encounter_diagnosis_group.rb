...
module PacioTestKit
  module PFE
    class ConditionEncounterDiagnosisGroup < Inferno::TestGroup
      ...
      The PACIO PFE Condition Encounter Diagnosis Profile tests verify that the system under test is able to provide
      correct responses for Condition queries. These queries must contain resources conforming to the
      Condition Encounter Diagnosis Profile as specified in the PACIO Personal Functioning and Engagement (PFE) IG
      v2.0.0 Implementation Guide.

      # Testing Methodology

      ## Reading
      The read interaction will perform required read associated with ID(s) provided by a user for Condition
      ...
      ## Profile Validation
      Each resource returned from the first read is expected to conform to the Condition Encounter Diagnosis Profile.
      Each element is checked against terminology binding and cardinality requirements. Elements with a required binding
